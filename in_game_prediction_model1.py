...
#flag = input('Enter either 0 for live in game probability or 1 for the win percentage')
...
#numbers from a linear regression I ran between point diff and win percentage from the start of the quarter
#win_percentage_point_diff = (.0153129791449*point_diff + 0.515353331477) *100
#print(win_percentage_point_diff)
...
if(time1 < one_min):
	...
	elif(point_diff == 3 and time1 >= one_min):
	...
	elif(point_diff == 4 and time1 >= one_min):
	...
	elif(point_diff == 5 and time1 >= one_min):
	...
	elif(point_diff == 6 and time1 >= one_min):
...
#[-0.25782663  0.01899752  0.88516012]

#[ 1.90492871 -1.57033168]
...
cur_game = data.game_id[154]

for i in range(154, length):
	...
	if(data.event_type[i] == 'Made Shot '):
		...
		if(isinstance(data.home_description[i], str) == True ):
			#home team
			win_percentage1 += ((time1 * (effective_field_goal) *(win_percentage1/2)) / point_diff)
	...
	#formula to increase/ decreaase win percentage based on current point diff, the time, and the inverse of NBA's average effective field goal percentage
		...
		if(isinstance(data.home_description[i], str) == True ):
			#home team
			win_percentage1 -= ((time1 * (1- effective_field_goal) * (win_percentage1/2)) / point_diff)
			if(point_diff_real > 0 and win_percentage1 > 50):
				win_percentage_array.append(win_percentage1)
				print(win_percentage1)
			elif(point_diff_real < 0 and win_percentage1 < 50):
	...
	#formula to increase/ decrease win percentage based on current point diff, the time, and the NBA's average defensive rebounding percentage
		...
		if(isinstance(data.home_description[i], str) == True ):
			#home team
			win_percentage1 += ((time1 * (1-def_rebound) * (win_percentage1/2)) / point_diff)
			if(point_diff_real > 0 and win_percentage1 > 50):
				win_percentage_array.append(win_percentage1)
				print(win_percentage1)
			elif(point_diff_real < 0 and win_percentage1 < 50):
	...
	#formula to increase/ decrease win percentage based on current point diff, the time, and the NBA's average turnover percentage
		...
		if(isinstance(data.home_description[i], str) == True ):
			#home team
			win_percentage1 -= ((time1 * ( turnover) * (win_percentage1/2)) / point_diff)
			if(point_diff_real > 0 and win_percentage1 > 50):
				win_percentage_array.append(win_percentage1)
				print(win_percentage1)
			elif(point_diff_real < 0 and win_percentage1 < 50):
	...
	#formula to increase/ decrease win percentage based on current point diff, the time, and the NBA's average free throw percentage
		...
		if(isinstance(data.home_description[i], str) == True and data.home_description[i].find("MISS") == -1 ):
			#home team
			win_percentage1 += ((time1 * (1- free_throw) * (win_percentage1/2)) / point_diff)
			if(point_diff_real > 0 and win_percentage1 > 50):
				win_percentage_array.append(win_percentage1)
				print(win_percentage1)
			elif(point_diff_real < 0 and win_percentage1 < 50):
